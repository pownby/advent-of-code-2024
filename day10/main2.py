...
def count_trails(row, col, grid, height):
  if (height == 9):
    return 1
  ...
  for i in range(4):
    ...
    if (is_valid(next_row, next_col, grid)):
      ...
      if (next_height == height + 1):
        count += count_trails(next_row, next_col, grid, next_height)
  return count
...
def main2():
  ...
  for row in range(row_count):
    for col in range(col_count):
      if grid[row][col] == 0:
        trail_count = count_trails(row, col, grid, 0)
        total_score += trail_count
